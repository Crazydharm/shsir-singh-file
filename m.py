...
bot = telebot.TeleBot('8154490232:AAH9bQ84LWJuVzOB6tt32ush4KwwmCeqmiU')
...
def start_attack_reply(message, target, port, time):
    ...
    response = f"🚀𝗬𝗢𝗨𝗥 𝗜𝗣 𝗖𝗛𝗔𝗡𝗚𝗘𝗦 𝗘𝗩𝗘𝗥𝗬 𝗦𝗘𝗖.😈"
...
def handle_bgmi(message):
    ...
    if user_id in allowed_user_ids:
        ...
        command = message.text.split()
        if len(command) == 4:  # BY :- @THE_GREAT_KING_DEVIL_BOT # Updated to accept target, time, and port
            target = command[1]
            port = int(command[2])  
            time = int(command[3])  
            if time > 800:
                response = "𝗘𝗿𝗿𝗼𝗿: 𝗧𝗶𝗺𝗲 𝗶𝗻𝘁𝗲𝗿𝘃𝗮𝗹 𝗺𝘂𝘀𝘁 𝗯𝗲 𝗹𝗲𝘀𝘀 𝘁𝗵𝗮𝗻 800."
            else:
...
def broadcast_message(message):
    ...
    if user_id in admin_id:
        ...
        response = "𝗢𝗡𝗟𝗬 𝗢𝗪𝗡𝗘𝗥 𝗖𝗔𝗡 𝗨𝗦𝗘 @THE_GREAT_KING_DEVIL_BOT"
